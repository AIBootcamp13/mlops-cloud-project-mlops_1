...
def run_temperature_train(data_root_path, model_root_path):
    epochs = 30
    window_size = 30
    ...
    trainer = BaselineTrainer("MULTI_OUTPUT_LSTM", epochs, outputs_temperature, scaler, window_size)
...
def run_pm_train(data_root_path, model_root_path):
    epochs = 30
    window_size = 30
    ...
    trainer = BaselineTrainer("MULTI_OUTPUT_LSTM", epochs, outputs_PM, scaler, window_size)
    trainer.split_data(data)
    model, val_loss = trainer.train_model()
    trainer.save_model(model_root_path, save_model_name, False)
