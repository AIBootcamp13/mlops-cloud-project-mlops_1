...
def interpolate_temperature(df_ta):
    df_ta_interpolated = df_ta.set_index('date').interpolate(method='time', limit_direction='both').reset_index()
...
def save_interpolated_temperature(df_ta_interpolated):
    ...
    for date_value, group in df_ta_interpolated.groupby('date'):
...
def save_processed_pm10(df_pm10_filtered):
    ...
    for date_value, group in df_pm10_filtered.groupby('date'):
        date_str = pd.to_datetime(date_value).strftime('%Y-%m-%d')
